...
#define EXTRACT_LEFTWARD_MOVE(_i, _gen, _result) \
  do { \
    ...
    u8 dest = _tzcnt_u64(dest_bit); \
    OFFSET(dest, _i); \
    u8 orig = 63 - _lzcnt_u64(_blsmsk_u64(dest_bit) & _gen->leftward_occ._[_i]); \
    OFFSET(orig, _i); \
    _result->orig = orig; \
    _result->dest = dest; \
    _gen->dests -= dest_bit; \
...
#define EXTRACT_LEFTWARD_MOVE_R(_i, _gen, _result) \
  do { \
    ...
    u8 dest = _tzcnt_u64(dest_bit); \
    OFFSET(dest, _i); \
    u8 orig = 63 - _lzcnt_u64(_blsmsk_u64(dest_bit) & _gen->leftward_occ_r._[_i]); \
    OFFSET(orig, _i); \
...
#define INIT_RIGHTWARD_MOVES(_i, _r, _gen) \
  do { \
    ...
    u64 movers_dep = _pdep_u64(movers_ext, (blockers << 1)); \
    _gen->move_mask = (_gen->movers##_r._[_i] - movers_dep) & _gen->targets##_r._[_i]; \
    _gen->origs = _gen->movers##_r._[_i] & ~(_gen->movers##_r._[_i] - _gen->targets##_r._[_i]); \
  } while(0)

// Get next origin for rightward moves and calculate its destinations (non-rotated)
#define GET_NEXT_RIGHTWARD_ORIGIN(_i, _gen) \
  do { \
    if (_gen->origs == 0) break; \
...
#define INIT_RIGHTWARD_CENTER_MOVES(_r, _gen) \
  (_gen->origs = _gen->center_movers##_r & ~(_gen->center_occ##_r - (GET_CENTER_ROW(_gen->targets##_r))))
  ...
  do { \
    ...
    u16 above_highest_occ_mask = \
        (_gen->center_occ##_r & below) \
            ...
            : (u16)-1; \
    _gen->dests = GET_CENTER_ROW(_gen->targets##_r) & below & above_highest_occ_mask; \
  } while(0)
  ...
  while (gen->state < GEN_DONE) {
    switch (gen->state) {
      ...
      case GEN_LEFTWARD_1_R_EXTRACT:
        ...
        gen->state = GEN_LEFTWARD_CENTER_R;
        break;
