...
static int replacement_priority(tt_entry *entry, u8 current_gen) {
  return (int)entry->depth - 4 * age_distance(current_gen, entry->flags.generation);
}
...
void tt_store(
  ...
  int victim = 0;
  int lowest_priority = replacement_priority(&bucket->entries[0], tt->generation);
...
void tt_prefetch(transposition_table *tt, u64 hash) {
  size_t index = hash & tt->mask;
  __builtin_prefetch(&tt->buckets[index], 0, 0);
}
