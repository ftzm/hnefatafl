...
// Board allocator/printer/type_info provided by test_util.h:
// theft_alloc_board, theft_print_board, theft_board_info_shrink
...
static enum theft_trial_res prop_board_printable(struct theft *t, void *arg1) {
  ...
  board_string_t s = to_board_string(*input);
  fprintf(devnull, "%s", s._);
  fclose(devnull);
  return THEFT_TRIAL_PASS;
}

PROP_TEST_DEFAULT(test_board_printable, prop_board_printable,
                  &theft_board_info_shrink)
...
void reference_dir_moves_black(
  ...
  while (true) {
    ...
    OP_LAYER_BIT(b2.black_r, rotate_right[pos], |=);
    {
      ...
      apply_captures_niave(friends, &b2.white, &b2.white_r, pos);
    }
    ms[(*cnt)].b = b2;
    (*cnt)++;
  }
...
static void print_dir_moves(
    FILE *f,
    const char *label,
    int count,
    struct move_set moves[20]) {
  for (int j = 0; j < count; j++)
    fprint_move(f, label, moves[j].m);
}
...
void diff_print_cb(FILE *f, const void *instance, void *env) {
  ...
  for (int i = 0; i < 121; i++) {
    print_dir_moves(f, "extra north", d->extra[i].north_count, d->extra[i].north);
    print_dir_moves(f, "extra east", d->extra[i].east_count, d->extra[i].east);
    print_dir_moves(f, "extra south", d->extra[i].south_count, d->extra[i].south);
    print_dir_moves(f, "extra west", d->extra[i].west_count, d->extra[i].west);
    print_dir_moves(f, "missing north", d->missing[i].north_count, d->missing[i].north);
    print_dir_moves(f, "missing east", d->missing[i].east_count, d->missing[i].east);
    print_dir_moves(f, "missing south", d->missing[i].south_count, d->missing[i].south);
    print_dir_moves(f, "missing west", d->missing[i].west_count, d->missing[i].west);
  }
};
...
PROP_TEST_DEFAULT(test_get_team_moves_black, prop_diff_empty,
                  &get_team_moves_black_info)
...
PROP_TEST_DEFAULT(test_get_team_moves_white, prop_diff_empty,
                  &get_team_moves_white_info)
...
PROP_TEST_DEFAULT(test_get_team_moves_king, prop_diff_empty,
                  &get_team_moves_king_info)
...
struct moves_diffs compare_moves(
  ...
  // Walk both sorted lists, incrementing only one if it "falls behind".
  ...
  // Collect remaining elements from whichever list isn't exhausted.
  while (a_index < a_len)
    d.a_excess[d.a_excess_len++] = a[a_index++];
  while (b_index < b_len)
    d.b_excess[d.b_excess_len++] = b[b_index++];
...
void moves_diffs_print_cb(FILE *f, const void *instance, void *env) {
  ...
  for (int i = 0; i < d->a_excess_len; i++)
    fprint_move(f, "missing", d->a_excess[i]);
  for (int i = 0; i < d->b_excess_len; i++)
    fprint_move(f, "extra", d->b_excess[i]);
}
...
prop_moves_diffs_empty(struct theft *t, void *arg1) {
  ...
  return (!input->a_excess_len && !input->b_excess_len) ? THEFT_TRIAL_PASS
                                                        : THEFT_TRIAL_FAIL;
}

// Macro to define a moves_diffs property test with a given allocator and
// trial count. Eliminates the repeated type_info + theft_run_config
// boilerplate that appears ~15 times in this file.
#define MOVES_DIFFS_TEST(test_name, alloc_cb, trials)                          \
  static struct theft_type_info test_name##_info = {                           \
      .alloc = alloc_cb,                                                       \
      .free = theft_generic_free_cb,                                           \
      .print = moves_diffs_print_cb,                                           \
      .autoshrink_config = {.enable = false},                                  \
  };                                                                           \
  PROP_TEST(test_name, prop_moves_diffs_empty, &test_name##_info, trials)
...
MOVES_DIFFS_TEST(test_reference_moves_black, create_reference_moves_black_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_reference_moves_white, create_reference_moves_white_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_mm_moves_white, create_mm_moves_white_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_mm_moves_black, create_mm_moves_black_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_mm_moves_king, create_mm_moves_king_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_moves_to_black, moves_to_black_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_moves_to_white, moves_to_white_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_moves_to_king, moves_to_king_cb,
                  1000)
...
// custom type_info for test_moves_to_layers_correct
static struct theft_type_info test_moves_to_layers_correct_info = {
...
PROP_TEST(test_moves_to_layers_correct, move_to_entries_empty, &test_moves_to_layers_correct_info, 100)
...
// custom type_info for test_white_moves_count
static struct theft_type_info test_white_moves_count_info = {
...
PROP_TEST(test_white_moves_count, prop_move_counts_equal, &test_white_moves_count_info, 100)
...
// custom type_info for test_black_moves_count
static struct theft_type_info test_black_moves_count_info = {
...
PROP_TEST(test_black_moves_count, prop_move_counts_equal, &test_black_moves_count_info, 100)
...
// custom type_info for test_king_moves_count
static struct theft_type_info test_king_moves_count_info = {
...
PROP_TEST(test_king_moves_count, prop_move_counts_equal, &test_king_moves_count_info, 100)
...
MOVES_DIFFS_TEST(test_moves_from_layers_black, moves_from_layers_black_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_moves_from_layers_white, moves_from_layers_white_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_generator_moves_black, generator_moves_black_cb,
                  1000)
...
MOVES_DIFFS_TEST(test_generator_moves_white, generator_moves_white_cb,
                  1000)
...
void validated_moves_print_cb(FILE *f, const void *instance, void *env) {
  ...
  board_string_t bs = to_board_string(vm->b);
  fprintf(f, "%s", bs._);
  fprintf(f, "Total moves: %d\n", vm->total);
}

// custom type_info for test_validate_black_moves
static struct theft_type_info test_validate_black_moves_info = {
...
PROP_TEST(test_validate_black_moves, prop_black_moves_valid, &test_validate_black_moves_info, 1000)
