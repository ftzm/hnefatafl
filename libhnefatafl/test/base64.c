...
alloc_test_data(struct theft *t, void *env, void **instance) {
  (void)env;
...
prop_encode_decode_roundtrip(struct theft *t, void *arg1) {
  (void)t;
...
TEST move_list_roundtrip() {
  ...
  move *decoded_single =
      move_list_from_base64(single_encoded, &decoded_count_single);
  ...
  move *decoded_multiple =
      move_list_from_base64(multiple_encoded, &decoded_count_multiple);
...
TEST move_list_invalid_base64() {
  ...
  int result_count;
  move *result_moves = move_list_from_base64(
      "QQ",
      &result_count); // Decodes to 1 byte, not even number
