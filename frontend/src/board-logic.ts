...
export function reduceMovesToTransformation(moves: Move[]): Transformation {
  const result = moves.reduce<Transformation>(
    (acc, move) => {
      const sortedCaptures = [...(move.captures || [])].sort((a, b) => a - b);
      const capturedOriginalPositions = sortedCaptures.map(
        (capturedPosition) => {
          const originalPosition = findKeyByValue(
            acc.pieceTransformations,
            capturedPosition,
          );
          if (originalPosition) {
      ...
      const origin =
        findKeyByValue(acc.pieceTransformations, move.from) || move.from;
...
export function reverseMovesToTransformation(
  moves: Move[],
  startColor: PlayerColor,
): ReverseTransformation {
  let currentColor: PlayerColor = startColor;

  const result = moves.reduceRight<ReverseTransformation>(
    (acc, move) => {
      const capturedColor: PlayerColor =
      ...
      const restorationIndex = acc.restorations.findIndex(
        (r) => r.index === move.to,
      );
      ...
      if (!acc.restorations.some((r) => r.index === move.from)) {
        const origin =
          findKeyByValue(acc.pieceTransformations, move.to) || move.to;
