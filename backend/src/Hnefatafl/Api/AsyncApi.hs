...
import Hnefatafl.Api.OpenApi () -- orphan ToSchema instances
import Hnefatafl.Api.Types.WS (AuthMessage)
...
schemaWithDefs :: forall a. ToSchema a => Proxy a -> (Referenced Schema, [(Text, Schema)])
  ...
  object
    ...
    , "info"
        .= object
          ...
          , "version" .= ("0.0.16" :: Text)
          , "description" .= ("WebSocket message schemas for AI and Online game modes" :: Text)
          ]
    , "channels"
        .= object
          [ "ai-ws"
        ...
        .= object
          [ "sendAiClient"
              .= object
                ...
                , "messages" .= [ref "#/channels/ai-ws/messages/clientMessage", ref "#/channels/ai-ws/messages/authMessage"]
                ]
          , "receiveAiServer"
              .= object
                [ "action" .= ("receive" :: Text)
                , "channel" .= ref "#/channels/ai-ws"
                , "messages" .= [ref "#/channels/ai-ws/messages/serverMessage"]
                ]
          , "sendOnlineClient"
              .= object
                [ "action" .= ("send" :: Text)
                , "channel" .= ref "#/channels/online-ws"
                , "messages" .= [ref "#/channels/online-ws/messages/clientMessage", ref "#/channels/online-ws/messages/authMessage"]
                ]
          , "receiveOnlineServer"
              .= object
                [ "action" .= ("receive" :: Text)
                , "channel" .= ref "#/channels/online-ws"
                , "messages" .= [ref "#/channels/online-ws/messages/serverMessage"]
                ]
          ]
    , "components"
        .= object
